...
// Material Design check_circle badge
function CheckmarkCircleBadge({ size, fillColor }: { size: number; fillColor: string }) {
  return (
    <svg
      width={size}
      height={size}
...
export const VerificationBadge: React.FC<VerificationBadgeProps> = ({
  ...
  const fillColor = level === 'provider' ? 'var(--color-status-verified)' : 'var(--color-accent)';
  ...
  return (
    <span
      ...
      aria-label={message}
    >
      <CheckmarkCircleBadge size={size} fillColor={fillColor} />
    </span>
  );
};
